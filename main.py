...
@Hydrix.on_message(filters.private & filters.command("start"))
async def start_message(bot, message):
    await message.reply_sticker(
        sticker="CAACAgIAAxkBAAIT5WISSkkuA1FvGA5S5_2kqtNrhPK3AAKQEwACINLgSyf6joo76oG1HgQ"             
    )
...
@Hydrix.on_message(filters.command("alert"))
async def alert(bot, message):
    await message.reply_text(
        text=f"hello {message.from_user.mention}",
        reply_markup=InlineKeyboardMarkup( [[
           InlineKeyboardButton("Alert 1", callback_data="alert_1")
           ]]
           )
       )
...
async def callback_data(bot, query):
    if query.data == "alert_1":
        await query.answer("hello", show_alert=False)

@Hydrix.on_message(filters.command("alert2"))
async def alert(bot, message):
    await message.reply_text(
        text=f"hello {message.from_user.mention}",
        reply_markup=InlineKeyboardMarkup( [[
           InlineKeyboardButton("Alert 2", callback_data="alert_2")
           ]]
           )
       )
@Hydrix.on_callback_query()
async def callback_data(bot, query):
    if query.data == "alert_2":
...
@Hydrix.on_message(filters.private & filters.command("info"))
...
@Hydrix.on_message(filters.group & filters.command("ginfo"))
...
@Hydrix.on_message(filters.command("id"))
async def id(bot, hydrix):
    await hydrix.reply_sticker(
...
@Hydrix.on_message(filters.private & filters.command("dc"))
